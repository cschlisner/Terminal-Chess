    ...
    private double minimax(Move move, int depth, double alpha, double beta, Game.Color color){
        ...
        if (color.equals(this.color)){
            ...
            for (Move opponentMove : responseMoves){
                v = Math.max(minimax(new SmartMove(opponentMove), depth-1, alpha, beta, Game.getOpposite(color)), v);
            ...
            for (Move opponentMove : responseMoves){
                v = Math.min(minimax(new SmartMove(opponentMove), depth-1, alpha, beta, Game.getOpposite(color)), v);
