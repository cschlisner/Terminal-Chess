	...
	// System.out.format("pieceSymbol: %s\ndest: %s\n", pieceSymbol, dest);
		...
		for (int y = 0; y < 8; ++y){
			for (int x = 0; x < 8; ++x){
                ...
                System.out.println(Piece.symbol(p)+" : "+ploc+" "+dest);

                if (p == Piece.NONE || Piece.color(p) != color ||
                        !Piece.symbol(p,false).equals(pieceSymbol)) {
                    continue;
                }
                Move pmove = new Move(ploc, dest, board);
                System.out.println(pmove+" : "+ploc+" "+dest);
                if (board.isValidMove(pmove)){
                    if (((rank < 0 ^ ploc.y == rank)) && ((file < 0 ^ ploc.x == file))) // if the rank or file have been specified, add only matching pieces
                    {
